...
if($in{'subfun'} eq "")
{
my $rcfolder='/etc/webmin/vdcsitemanager/resource-group/activelist/';
my $cmdmk="mkdir -p ".$rcfolder."";
my $cmdoutmk=`$cmdmk`;
...
my @recgroup=();
my $ai=0;
my $hlock = '/etc/webmin/vdcsitemanager/resource-group/activelist/';  # Replace this with your folder path
opendir(my $dh, $hlock) or die "Cannot open directory: $!";

while (my $file = readdir($dh)) {
    next unless (-f "$hlock/$file");  # Check if it's a file
    if ($file =~ /\.conf$/) {  # Check for files with the .local extension
#print "<hr> $file";
my @vx=split("-",$file);
$recgroup[$ai]{'filename'}=>$file;
$recgroup[$ai]{'gid'}=$vx[0];
...
print "<table border=1>";
print "<tr>";
print "<td style=\"border: 1px solid;background-color:#bbbbbb !important\" align=center><a href=\"index.cgi?fun=resourcegroup&getsortby=vmid\">GID</a></td>";
print "<td style=\"border: 1px solid;background-color:#bbbbbb !important\" align=center><a href=\"index.cgi?fun=resourcegroup&getsortby=fromnodeip\">RESOURCE GROUP NAME</a></td>";
print "<td style=\"border: 1px solid;background-color:#bbbbbb !important\" align=center><a href=\"index.cgi?fun=resourcegroup&getsortby=tonodeip\">NO OF VMs</a></td>";
print "<td style=\"border: 1px solid;background-color:#bbbbbb !important\" align=center><a href><a href=\"index.cgi?fun=resourcegroup&getsortby=starttime\">SCHEDULE TIME</a></td>";
print "<td style=\"border: 1px solid;background-color:#bbbbbb !important\" align=center><a href=\"index.cgi?fun=resourcegroup&getsortby=endtime\">SCHEDULE ACTIVE</a></td>";
print "</tr>";
...
for($ai=0;$ai<@recgroup;$ai++)
{
my $gid=$recgroup[$ai]{'gid'};
my $groupname=$recgroup[$ai]{'groupname'};
my $vmcount=$recgroup[$ai]{'vmcount'};
my $crontime=$recgroup[$ai]{'crontime'};
my $cronactive=$recgroup[$ai]{'cronactive'};
if($vmcount eq ""){$vmcount=0;}
if($crontime eq ""){$crontime="-";}
if($cronactive eq ""){$cronactive="-";}
print "<tr>";
print "<td style=\"border: 1px solid;background-color:#cceecc !important\" align=center>".$gid."</td>";
print "<td style=\"border: 1px solid;background-color:#cceecc !important\" align=center>".$groupname."</td>";
print "<td style=\"border: 1px solid;background-color:#cceecc !important\" align=center>".$vmcount."</td>";
...
print "</table>";
print "<br><br><a href=\"index.cgi?fun=resourcegroup&subfun=create\" style=\"display: inline-block; padding: 10px 20px; background-color: #007bff; color: #fff; text-decoration: none; border-radius: 5px; border: 1px solid #007bff;\">Create Resource-Group</a>";  
print "<br><br>&nbsp;";
### show list of recorcegroup list over
}
